...
export function AccessControlPage() {
  ...
  const availablePermissions: AccessControlPermission[] = PLANNED_STAFFING_ENABLED
    ? permissions
    : permissions.filter(
        (permission: AccessControlPermission): boolean =>
          !isPlannedStaffingPermission(permission.code),
      );
    ...
    <div className="space-y-6">
      ...
      {tab === "users" && userEditor ? (
        <div className="grid gap-6 xl:grid-cols-[300px_minmax(0,1fr)]">
          ...
          <section className="surface-card p-5 sm:p-6"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><h2 className="text-lg font-bold">{users.find((user: AccessControlUser): boolean => user.account_id === userEditor.account_id)?.username}</h2><p className="mt-1 text-sm text-slate-500">Gán vai trò trên toàn doanh nghiệp hoặc theo từng chi nhánh; quyền từ chi nhánh khác không được cộng vào.</p></div><label className="text-xs font-semibold text-slate-500">Vai trò tổ chức chính<select className="mt-1 block min-h-10 rounded-xl border-slate-300 text-sm text-slate-900" value={userEditor.primary_role} onChange={(event): void => setUserEditor({ ...userEditor, primary_role: event.target.value })}>{activeRoles.filter((role: AccessControlRole): boolean => role.is_system).map((role: AccessControlRole): React.ReactNode => <option key={role.code} value={role.code}>{role.display_name}</option>)}</select></label></div>
