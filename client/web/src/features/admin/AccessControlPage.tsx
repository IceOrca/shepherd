...
function permissionGroup(code: PermissionCode): string {
  const parts: string[] = code.split(".");
  return parts.length > 1 ? `${parts[0]}.${parts[1]}` : parts[0] ?? "other";
}
...
export function AccessControlPage() {
  ...
  const groupedPermissions: Map<string, AccessControlPermission[]> = useMemo(
    (): Map<string, AccessControlPermission[]> => {
      const groups: Map<string, AccessControlPermission[]> = new Map<string, AccessControlPermission[]>();
      for (const permission of permissions) {
        const group: string = permissionGroup(permission.code);
        const current: AccessControlPermission[] = groups.get(group) ?? [];
    ...
    <div className="space-y-6">
      <section className="surface-card p-5 sm:p-6">
        <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-blue-600">Tenant access control</p>
            <h1 className="mt-2 text-2xl font-bold text-slate-950">Phân quyền doanh nghiệp</h1>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-500">Vai trò và quyền thuộc riêng tenant. Quyền theo chi nhánh chỉ có hiệu lực khi người dùng chọn đúng chi nhánh đó.</p>
          </div>
        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_380px]">
          ...
          <div className="space-y-6">
            <form className="surface-card space-y-4 p-5" onSubmit={submitBranch}>
              ...
              <input className="min-h-11 rounded-xl border-slate-300" placeholder="Mã chi nhánh" required value={branchRequest.code} onChange={(event): void => setBranchRequest({ ...branchRequest, code: event.target.value })} />
              <input className="min-h-11 rounded-xl border-slate-300" placeholder="Tên chi nhánh" required value={branchRequest.name} onChange={(event): void => setBranchRequest({ ...branchRequest, name: event.target.value })} />
              <input className="min-h-11 rounded-xl border-slate-300" placeholder="IANA time zone" required value={branchRequest.time_zone} onChange={(event): void => setBranchRequest({ ...branchRequest, time_zone: event.target.value })} />
      ...
      {tab === "roles" ? (
        <div className="grid gap-6 xl:grid-cols-[300px_minmax(0,1fr)]">
          <div className="space-y-6">
            <section className="surface-card overflow-hidden"><div className="border-b border-slate-100 px-5 py-4 font-bold">Vai trò</div>{roles.map((role: AccessControlRole): React.ReactNode => <button className={`block w-full border-b border-slate-100 px-5 py-3 text-left ${selectedRoleCode === role.code ? "bg-blue-50 text-blue-800" : "hover:bg-slate-50"}`} key={role.code} onClick={(): void => setSelectedRoleCode(role.code)} type="button"><span className="block font-bold">{role.display_name}</span><span className="mt-1 block text-xs opacity-70">{role.scope === "tenant" ? "Toàn tenant" : "Theo chi nhánh"} · {role.assigned_account_count} người</span></button>)}</section>
            <form className="surface-card space-y-3 p-5" onSubmit={submitRole}><h2 className="font-bold">Tạo vai trò tùy chỉnh</h2><input className="min-h-10 rounded-xl border-slate-300" placeholder="role_code" required value={roleRequest.code} onChange={(event): void => setRoleRequest({ ...roleRequest, code: event.target.value })} /><input className="min-h-10 rounded-xl border-slate-300" placeholder="Tên hiển thị" required value={roleRequest.display_name} onChange={(event): void => setRoleRequest({ ...roleRequest, display_name: event.target.value })} /><select className="min-h-10 rounded-xl border-slate-300" value={roleRequest.scope} onChange={(event): void => setRoleRequest({ ...roleRequest, scope: event.target.value as AccessRoleScope })}><option value="branch">Theo chi nhánh</option><option value="tenant">Toàn tenant</option></select><button className="action-primary w-full" type="submit"><Plus className="size-4" />Tạo vai trò</button></form>
          </div>
          {roleEditor ? <section className="surface-card p-5 sm:p-6"><div className="grid gap-4 sm:grid-cols-2"><label className="block"><span className="text-sm font-bold">Tên vai trò</span><input className="mt-2 min-h-11 rounded-xl border-slate-300" value={roleEditor.display_name} onChange={(event): void => setRoleEditor({ ...roleEditor, display_name: event.target.value })} /></label><label className="block"><span className="text-sm font-bold">Mã / phạm vi</span><div className="mt-2 min-h-11 rounded-xl bg-slate-100 px-3 py-3 text-sm">{roleEditor.code} · {roles.find((role: AccessControlRole): boolean => role.code === roleEditor.code)?.scope}</div></label></div><label className="mt-4 block"><span className="text-sm font-bold">Mô tả</span><textarea className="mt-2 min-h-20 rounded-xl border-slate-300" value={roleEditor.description} onChange={(event): void => setRoleEditor({ ...roleEditor, description: event.target.value })} /></label><div className="mt-6 space-y-5">{Array.from(groupedPermissions.entries()).map(([group, groupPermissions]: [string, AccessControlPermission[]]): React.ReactNode => <fieldset key={group}><legend className="text-sm font-bold uppercase tracking-wide text-slate-500">{group}</legend><div className="mt-2 grid gap-2 md:grid-cols-2">{groupPermissions.map((permission: AccessControlPermission): React.ReactNode => <label className="flex items-start gap-3 rounded-xl border border-slate-200 p-3 text-sm" key={permission.code}><input checked={roleEditor.permission_codes.includes(permission.code)} className="mt-1" onChange={(): void => toggleRolePermission(permission.code)} type="checkbox" /><span><span className="block font-semibold text-slate-800">{permission.code}</span><span className="mt-1 block text-xs leading-5 text-slate-500">{permission.description}</span></span></label>)}</div></fieldset>)}</div><div className="mt-6 flex items-center justify-between gap-4"><label className="flex items-center gap-2 text-sm font-semibold"><input checked={roleEditor.is_active} disabled={roles.find((role: AccessControlRole): boolean => role.code === roleEditor.code)?.is_system} onChange={(event): void => setRoleEditor({ ...roleEditor, is_active: event.target.checked })} type="checkbox" />Đang hoạt động</label><button className="action-primary" disabled={roleUpdateMutation.isPending} onClick={(): void => roleUpdateMutation.mutate(roleEditor)} type="button"><Save className="size-4" />Lưu vai trò và quyền</button></div></section> : null}
        </div>
      ) : null}
