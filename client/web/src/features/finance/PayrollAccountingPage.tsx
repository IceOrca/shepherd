...
function FinancialPeriodDialog({
  ...
  const targetStatus: FinancialPeriodChangeRequest["status"] =
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/50 p-4">
      ...
      >
        <div className="flex items-start gap-4">
          <div
            className={`grid size-11 shrink-0 place-items-center rounded-xl ${targetStatus === "closed" ? "bg-red-50 text-red-700" : "bg-emerald-50 text-emerald-700"}`}
          >
            {targetStatus === "closed" ? (
          ...
          <div>
            <h2 className="text-xl font-black text-slate-950">
              {targetStatus === "closed" ? "Khóa" : "Mở lại"}{" "}
        ...
        <label className="mt-5 block text-sm font-semibold text-slate-700">
          Lý do
          <textarea
            className="mt-2 min-h-28 w-full rounded-xl border-slate-300"
            maxLength={500}
            minLength={3}
        ...
        {error ? (
          <p className="mt-3 text-sm font-semibold text-red-700">
            {friendlyApiError(
              error,
              "Không thể khóa kỳ. Hãy xử lý giờ làm bị trùng nguồn, dữ liệu chưa hoàn tất hoặc thay đổi đồng thời rồi thử lại.",
            )}
          </p>
        ) : null}
...
function formatMoney(value: string, currency: string): string {
  ...
  const whole: string = (absolute / 10_000n)
    .toString()
    .replace(/\B(?=(\d{3})+(?!\d))/g, ".");
...
function sumByCurrency<T>(
  ...
  return totals.size === 0
    ? "0 VND"
    : [...totals.entries()]
        .map(([code, total]): string => formatMoney(decimalAmount(total), code))
        .join(" · ");
}

function aggregateFinancialLines(
  ...
  const fields: Array<keyof Omit<OperatingFinancialLine, "currency">> = [
    ...
    "operating_cost",
    "operating_profit",
    "business_profit_after_profit_share",
    "reimbursed_cash",
  ...
  for (const line of reports.flatMap(
    (report): OperatingFinancialLine[] => report.lines,
  )) {
  ...
  return [...totals.entries()].map(
    ([currency, values]): OperatingFinancialLine => ({
      currency,
      ...
      coordination_salary_cost: decimalAmount(
        values.coordination_salary_cost ?? 0n,
      ),
      approved_business_expense: decimalAmount(
        values.approved_business_expense ?? 0n,
      ),
      ...
      business_profit_after_profit_share: decimalAmount(
        values.business_profit_after_profit_share ?? 0n,
      ),
      reimbursed_cash: decimalAmount(values.reimbursed_cash ?? 0n),
      salary_advance_disbursed: decimalAmount(
        values.salary_advance_disbursed ?? 0n,
      ),
      salary_advance_recovered: decimalAmount(
        values.salary_advance_recovered ?? 0n,
      ),
      outstanding_expense_reimbursement: decimalAmount(
        values.outstanding_expense_reimbursement ?? 0n,
      ),
      outstanding_salary_advance: decimalAmount(
        values.outstanding_salary_advance ?? 0n,
      ),
    }),
  );
}
...
function aggregatePayrollLines(reports: PayrollReport[]): PayrollLine[] {
  ...
  for (const line of reports.flatMap((report): PayrollLine[] => report.lines)) {
    ...
    totals.set(key, {
      ...current,
      staffing_worked_seconds:
        current.staffing_worked_seconds + line.staffing_worked_seconds,
      staffing_earnings: decimalAmount(
        scaledAmount(current.staffing_earnings) +
          scaledAmount(line.staffing_earnings),
      ),
      prorated_monthly_salary: decimalAmount(
        scaledAmount(current.prorated_monthly_salary) +
          scaledAmount(line.prorated_monthly_salary),
      ),
      profit_share_base: decimalAmount(
        scaledAmount(current.profit_share_base) +
          scaledAmount(line.profit_share_base),
      ),
      profit_share_percent:
        scaledAmount(current.profit_share_percent) >=
        scaledAmount(line.profit_share_percent)
          ? current.profit_share_percent
          : line.profit_share_percent,
      profit_share_payment: decimalAmount(
        scaledAmount(current.profit_share_payment) +
          scaledAmount(line.profit_share_payment),
      ),
      profit_share_locked:
        current.profit_share_locked && line.profit_share_locked,
      gross_pay: decimalAmount(
        scaledAmount(current.gross_pay) + scaledAmount(line.gross_pay),
      ),
      recorded_expense_reimbursement: decimalAmount(
        scaledAmount(current.recorded_expense_reimbursement) +
          scaledAmount(line.recorded_expense_reimbursement),
      ),
      suggested_expense_reimbursement: decimalAmount(
        scaledAmount(current.suggested_expense_reimbursement) +
          scaledAmount(line.suggested_expense_reimbursement),
      ),
      recorded_advance_deduction: decimalAmount(
        scaledAmount(current.recorded_advance_deduction) +
          scaledAmount(line.recorded_advance_deduction),
      ),
      outstanding_advance_due: decimalAmount(
        scaledAmount(current.outstanding_advance_due) +
          scaledAmount(line.outstanding_advance_due),
      ),
      suggested_advance_deduction: decimalAmount(
        scaledAmount(current.suggested_advance_deduction) +
          scaledAmount(line.suggested_advance_deduction),
      ),
      estimated_net_pay: decimalAmount(
        scaledAmount(current.estimated_net_pay) +
          scaledAmount(line.estimated_net_pay),
      ),
      attendance_overlap_count:
        current.attendance_overlap_count + line.attendance_overlap_count,
    });
  }
  return [...totals.values()].sort(
    (left: PayrollLine, right: PayrollLine): number =>
...
export function PayrollAccountingPage(): React.JSX.Element {
  ...
  const canReadFinancial: boolean = permissions.includes(
    "finance.operating_reports.read",
  );
  const canReadPayroll: boolean = permissions.includes("hr.payroll.read");
  const canExportFinancial: boolean = permissions.includes(
  ...
  const canManageSalary: boolean = permissions.includes(
  ...
  const validRange: boolean = Boolean(
    startDate && endDate && startDate <= endDate,
  );
  const activeTab: ReportTab =
    tab === "financial" && !canReadFinancial
      ? canReadPayroll
        ? "payroll"
        : "salary"
  ...
  const financialQuery = useQuery({
    queryKey: [
      ...financeQueryKeys.operatingReport,
      reportScopeKey,
      startDate,
      endDate,
    ],
    queryFn: (): Promise<OperatingFinancialReport[]> =>
      Promise.all(
        reportBranchIds.map(
          (branchId: string): Promise<OperatingFinancialReport> =>
  ...
  const payrollQuery = useQuery({
    queryKey: [
      ...financeQueryKeys.payrollReport,
      reportScopeKey,
      startDate,
      endDate,
    ],
    queryFn: (): Promise<PayrollReport[]> =>
      Promise.all(
  ...
  const salaryQuery = useInfiniteQuery({
    queryKey: [...financeQueryKeys.salaryConfigurations, activeBranchId],
    initialPageParam: null as string | null,
    queryFn: ({ pageParam }) => listSalaryConfigurations(pageParam),
    getNextPageParam: (page) => page.next_cursor ?? undefined,
    enabled: canReadSalary && activeBranchId !== null,
  });
  const salaryConfigurations: EmployeeSalaryConfig[] =
    salaryQuery.data?.pages.flatMap((page) => page.items) ?? [];
  const periodsQuery = useQuery({
    queryKey: [
      ...financeQueryKeys.financialPeriods,
      activeBranchId,
      startDate,
      endDate,
    ],
    queryFn: (): Promise<FinancialPeriodState[]> =>
      listFinancialPeriodsForBranch(activeBranchId ?? "", startDate, endDate),
  ...
  const salaryMutation = useMutation({
    ...
    onSuccess: (record: EmployeeSalaryConfig): void => {
      ...
      void queryClient.invalidateQueries({
        queryKey: financeQueryKeys.payrollReport,
      });
      void queryClient.invalidateQueries({
        queryKey: financeQueryKeys.operatingReport,
      });
  ...
  >({
    ...
    onSuccess: (period: FinancialPeriodState): void => {
      ...
      void queryClient.invalidateQueries({
        queryKey: financeQueryKeys.salaryAdvances,
      });
      void queryClient.invalidateQueries({
        queryKey: financeQueryKeys.operatingReport,
      });
      ...
      setFeedback(
        `${monthLabel(period.period_start)} đã được ${period.status === "closed" ? "khóa; chi hộ và tạm ứng còn lại đã tự động tính vào lương" : "mở lại"}.`,
      );
    },
  });
  const exportMutation = useMutation<DownloadedFile, Error, ReportExportKind>({
    mutationFn: (reportKind: ReportExportKind): Promise<DownloadedFile> =>
      downloadFinancialReport({
    ...
    onSuccess: (file: DownloadedFile, reportKind: ReportExportKind): void => {
      ...
      setFeedback(
        reportKind === "payroll"
          ? "Đã tạo và tải bảng lương Excel."
          : "Đã tạo và tải báo cáo tài chính Excel.",
      );
    },
  });

  const financialLines: OperatingFinancialLine[] = aggregateFinancialLines(
    financialQuery.data ?? [],
  );
  const payrollLines: PayrollLine[] = aggregatePayrollLines(
    payrollQuery.data ?? [],
  );
  const overlapCount: number = payrollLines.reduce(
    (total: number, line: PayrollLine): number =>
      total + line.attendance_overlap_count,
    0,
  );

  if (!canReadFinancial && !canReadPayroll && !canReadSalary) {
    return (
      <section className="panel p-8 text-center font-bold text-slate-900">
        Bạn không có quyền xem lương và báo cáo tài chính.
      </section>
    );
  }
  ...
  return (
    <section className="space-y-5">
      <div className="panel grid gap-4 p-5 sm:grid-cols-2 xl:grid-cols-[1fr_1fr_1fr_1.2fr]">
        ...
        {!validRange ? (
          <p className="text-sm font-semibold text-red-600 sm:col-span-2 xl:col-span-4">
            Khoảng ngày báo cáo không hợp lệ.
          </p>
        ) : null}
        {activeTab !== "salary" &&
        ((activeTab === "financial" && canExportFinancial) ||
          (activeTab === "payroll" && canExportPayroll)) ? (
          <div className="flex flex-col gap-2 border-t border-slate-200 pt-4 sm:col-span-2 sm:flex-row sm:items-center sm:justify-between xl:col-span-4">
            <p className="text-sm text-slate-500">
              File Excel dùng đúng khoảng ngày và phạm vi chi nhánh đang chọn.
              Trạng thái kỳ và các cảnh báo được ghi trong file.
            </p>
            <button
              ...
              disabled={
                ...
                (activeTab === "financial"
                  ? financialQuery.isPending || financialQuery.isError
                  : payrollQuery.isPending || payrollQuery.isError)
              }
              onClick={(): void => {
                if (
                  activeTab === "payroll" &&
                  overlapCount > 0 &&
                  !window.confirm(
                    "Bảng lương còn khoảng làm việc trùng nguồn. Bạn vẫn muốn xuất file để kiểm tra?",
                  )
                )
                  return;
                exportMutation.reset();
                exportMutation.mutate(
                  activeTab === "payroll" ? "payroll" : "operating_financial",
                );
              }}
              type="button"
            >
        ...
        {exportMutation.error ? (
          <p className="text-sm font-semibold text-red-700 sm:col-span-2 xl:col-span-4">
            {friendlyApiError(
              exportMutation.error,
              "Không thể tạo file Excel. Vui lòng thu hẹp khoảng ngày hoặc phạm vi chi nhánh rồi thử lại.",
            )}
          </p>
        ) : null}
      </div>
      ...
      <div className="panel flex flex-wrap gap-2 p-2">
        {canReadFinancial ? (
          <button
            className={`flex-1 rounded-xl px-4 py-3 text-sm font-bold ${activeTab === "financial" ? "bg-slate-950 text-white" : "text-slate-600 hover:bg-slate-100"}`}
            onClick={(): void => setTab("financial")}
            type="button"
          >
        ...
        {canReadPayroll ? (
          <button
            className={`flex-1 rounded-xl px-4 py-3 text-sm font-bold ${activeTab === "payroll" ? "bg-slate-950 text-white" : "text-slate-600 hover:bg-slate-100"}`}
            onClick={(): void => setTab("payroll")}
            type="button"
          >
        ...
        {canReadSalary ? (
          <button
            className={`flex-1 rounded-xl px-4 py-3 text-sm font-bold ${activeTab === "salary" ? "bg-slate-950 text-white" : "text-slate-600 hover:bg-slate-100"}`}
            onClick={(): void => setTab("salary")}
            type="button"
          >
            <Settings2 className="mr-2 inline size-4" />
            Cấu hình lương tháng
          </button>
        ) : null}
      </div>
        ...
        <div className="panel p-5">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <h2 className="font-black text-slate-950">
                Kỳ lương và tài chính ·{" "}
                {scope.branches.find(
                  (branch): boolean => branch.id === activeBranchId,
                )?.name ?? "Chi nhánh đang chọn"}
              </h2>
              <p className="mt-1 text-sm text-slate-500">
                Khi khóa kỳ, hệ thống chốt thưởng theo lợi nhuận và số tiền thực
                trả, đồng thời xử lý số dư chi hộ, tạm ứng có ngày tính lương
                trong tháng. Mở lại không xóa các khoản đã chốt.
              </p>
            </div>
            {scopeMode === "tenant" ? (
              <span className="rounded-full bg-amber-50 px-3 py-1 text-xs font-bold text-amber-800">
                Chỉ áp dụng cho chi nhánh đang chọn
              </span>
            ) : null}
          </div>
          {periodsQuery.isPending ? (
            <div className="mt-4 flex items-center gap-2 text-sm text-slate-500">
              <LoaderCircle className="size-4 animate-spin" />
              Đang tải trạng thái kỳ...
            </div>
          ) : periodsQuery.error ? (
            <p className="mt-4 text-sm font-semibold text-red-700">
              {friendlyApiError(
                periodsQuery.error,
                "Không thể tải trạng thái kỳ tài chính.",
              )}
            </p>
          ) : (
            <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {(periodsQuery.data ?? []).map(
                (period: FinancialPeriodState): React.JSX.Element => (
                  <article
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="mt-1 text-xs text-slate-500">
                          Phiên bản {period.revision_number || "mặc định"}
                          {period.actor_username
                            ? ` · ${period.actor_username}`
                            : ""}
                        </p>
                      </div>
                      <span
                        className={`rounded-full px-2 py-1 text-xs font-bold ${period.status === "closed" ? "bg-red-50 text-red-700" : "bg-emerald-50 text-emerald-700"}`}
                      >
      ...
      {activeTab === "financial" && canReadFinancial ? (
        financialQuery.isPending ? (
          ...
          <div className="panel p-5 text-sm text-red-700">
            {friendlyApiError(
              financialQuery.error,
              "Không thể tính báo cáo tài chính.",
            )}
          </div>
        ) : (
          <>
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              <div className="panel flex items-center gap-4 p-5">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xl font-black text-slate-950">
                    {sumByCurrency(
                      financialLines,
                      (line): string => line.staffing_revenue,
              ...
              <div className="panel flex items-center gap-4 p-5">
                ...
                <div>
                  <p className="text-sm font-semibold text-slate-500">
                    Chi phí vận hành (không gồm thưởng)
                  </p>
                  <p className="mt-1 text-xl font-black text-slate-950">
                    {sumByCurrency(
                      financialLines,
                      (line): string => line.operating_cost,
                      (line): string => line.currency,
                    )}
                  </p>
                </div>
              ...
              <div className="panel flex items-center gap-4 p-5">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xl font-black text-slate-950">
                    {sumByCurrency(
                      financialLines,
                      (line): string => line.operating_profit,
                      (line): string => line.currency,
                    )}
                  </p>
                </div>
              ...
              <div className="panel flex items-center gap-4 p-5">
                <div className="grid size-12 place-items-center rounded-2xl bg-violet-50 text-violet-700">
                  <CircleDollarSign className="size-6" />
                </div>
                <div>
                  <p className="text-sm font-semibold text-slate-500">
                  ...
                  <p className="mt-1 text-xl font-black text-slate-950">
                    {sumByCurrency(
                      financialLines,
                      (line): string => line.business_profit_after_profit_share,
                      (line): string => line.currency,
                    )}
                  </p>
                </div>
              ...
              <table className="min-w-full text-sm">
                <thead className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500">
                  <tr>
                    ...
                    <th className="px-5 py-4">Tiền công Staff</th>
                    <th className="px-5 py-4">Lương quản lý</th>
                    <th className="px-5 py-4">Chi phí khác</th>
                    <th className="px-5 py-4">
                      Thưởng theo lợi nhuận (tách riêng)
                    </th>
                    <th className="px-5 py-4">Lợi nhuận vận hành</th>
                    <th className="px-5 py-4">
                      Lợi nhuận doanh nghiệp sau chia
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {(financialQuery.data ?? []).flatMap(
                    (report: OperatingFinancialReport): React.JSX.Element[] =>
                      report.lines.map(
                        (line: OperatingFinancialLine): React.JSX.Element => (
                          <tr key={`${report.branch_id}:${line.currency}`}>
                            ...
                            <td
                              className={`px-5 py-4 font-black ${scaledAmount(line.operating_profit) < 0n ? "text-red-700" : "text-emerald-700"}`}
                            >
                              {formatMoney(
                            ...
                            <td
                              className={`px-5 py-4 font-black ${scaledAmount(line.business_profit_after_profit_share) < 0n ? "text-red-700" : "text-violet-700"}`}
                            >
                              {formatMoney(
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              {financialLines.map(
                (line: OperatingFinancialLine): React.JSX.Element => (
                  <div className="panel p-5" key={line.currency}>
                    <h3 className="font-black text-slate-950">
                      Dòng tiền và số dư · {line.currency}
                    </h3>
                    <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
                      <div>
                        <dt className="text-slate-500">
                          Đã hoàn chi hộ trong kỳ
                        </dt>
                        <dd className="font-bold">
                          {formatMoney(line.reimbursed_cash, line.currency)}
                        </dd>
                      </div>
                      <div>
                        <dt className="text-slate-500">
                    ...
                    <p className="mt-4 text-xs text-slate-500">
                      Hoàn chi hộ và tạm ứng là dòng tiền hoặc thanh toán công
                      nợ, không được tính lại thành chi phí.
                    </p>
                  </div>
                ),
              )}
      ...
      {activeTab === "payroll" && canReadPayroll ? (
        payrollQuery.isPending ? (
          ...
          <div className="panel p-5 text-sm text-red-700">
            {friendlyApiError(payrollQuery.error, "Không thể tính bảng lương.")}
          </div>
        ) : (
          <>
            {overlapCount > 0 ? (
              <div className="flex gap-3 rounded-2xl border border-red-200 bg-red-50 p-4 text-sm text-red-800">
                ...
                <div>
                  <p className="font-black">
                    Có {overlapCount} khoảng làm việc bị trùng nguồn
                  </p>
                  <p className="mt-1">
                    Cần xử lý phần giao nhau giữa công việc khách hàng và chấm
                    công nội bộ trước khi dùng bảng này để trả lương.
                  </p>
                </div>
              </div>
            ) : null}
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              ...
              <div className="panel p-5">
                <p className="text-sm font-semibold text-slate-500">Thực trả</p>
                <p className="mt-1 text-xl font-black text-emerald-700">
                  {sumByCurrency(
                    payrollLines,
                    (line): string => line.estimated_net_pay,
                    (line): string => line.currency,
                  )}
                </p>
              </div>
              ...
              <table className="min-w-full text-sm">
                <thead className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500">
                  <tr>
                    ...
                    <th className="px-5 py-4">Nguồn lương</th>
                    <th className="px-5 py-4">Thưởng theo lợi nhuận</th>
                    <th className="px-5 py-4">Lương gộp</th>
                    <th className="px-5 py-4">Hoàn chi hộ</th>
                    <th className="px-5 py-4">Khấu trừ tạm ứng</th>
                    <th className="px-5 py-4">Thực trả</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {payrollLines.map((line: PayrollLine): React.JSX.Element => (
                    <tr
                      className={
                        line.attendance_overlap_count > 0 ? "bg-red-50/60" : ""
                      }
                      key={`${line.employee_id}:${line.currency}`}
                    >
                      <td className="px-5 py-4">
                        <p className="font-bold text-slate-950">
                          {line.employee_name}
                        ...
                        <p className="mt-1 text-xs text-slate-500">
                          {line.employee_code} · {roleLabel(line.role)} ·{" "}
                          {scopeMode === "tenant"
                            ? "Toàn doanh nghiệp"
                            : (scope.branches.find(
                                (branch): boolean =>
                                  branch.id === line.branch_id,
                              )?.name ?? "Chi nhánh")}
                        </p>
                        {line.attendance_overlap_count > 0 ? (
                          <p className="mt-1 text-xs font-bold text-red-700">
                            Trùng {line.attendance_overlap_count} khoảng chấm
                            công
                          </p>
                        ) : null}
                      </td>
                      <td className="px-5 py-4">
                        <p>
                          Tiền công:{" "}
                          {formatMoney(line.staffing_earnings, line.currency)} ·{" "}
                          {hours(line.staffing_worked_seconds)}
                        </p>
                        <p className="mt-1">
                          Lương tháng phân bổ:{" "}
                          {formatMoney(
                            line.prorated_monthly_salary,
                            line.currency,
                          )}
                        </p>
                      </td>
                      <td className="px-5 py-4">
                        <p className="font-black text-violet-700">
                          {formatMoney(
                            line.profit_share_payment,
                            line.currency,
                          )}
                        </p>
                        <p className="mt-1 text-xs text-slate-500">
                          {formatPercent(line.profit_share_percent)} của{" "}
                          {formatMoney(line.profit_share_base, line.currency)}
                        </p>
                        {scaledAmount(line.profit_share_percent) > 0n ? (
                          <span
                            className={`mt-2 inline-flex rounded-full px-2 py-1 text-[11px] font-bold ${line.profit_share_locked ? "bg-slate-200 text-slate-700" : "bg-amber-50 text-amber-800"}`}
                          >
                            {line.profit_share_locked ? "Đã khóa" : "Tạm tính"}
                          </span>
                        ) : null}
                      </td>
                      <td className="px-5 py-4 font-bold">
                        {formatMoney(line.gross_pay, line.currency)}
                      </td>
                      <td className="px-5 py-4">
                        <p>
                          Đã tính:{" "}
                      ...
                      <td className="px-5 py-4">
                        <p>
                          Đã tính:{" "}
                          {formatMoney(
                            line.recorded_advance_deduction,
                            line.currency,
                          )}
                        </p>
                        <p className="mt-1 font-bold text-violet-700">
                          Khi khóa:{" "}
                          {formatMoney(
                            line.suggested_advance_deduction,
                            line.currency,
                          )}
                        </p>
                      </td>
                      <td className="px-5 py-4 font-black text-emerald-700">
                        {formatMoney(line.estimated_net_pay, line.currency)}
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      {activeTab === "salary" && canReadSalary ? (
        <div className="space-y-5">
          {canManageSalary ? (
            <form
              className="panel grid gap-4 p-5 lg:grid-cols-[1.5fr_1fr_0.6fr_1fr_auto] lg:items-end"
              onSubmit={(event: FormEvent<HTMLFormElement>): void => {
                event.preventDefault();
                salaryMutation.mutate(salaryDraft);
              }}
            >
              <label className="text-sm font-semibold text-slate-700">
                Nhân viên quản lý
                <select
                  ...
                  required
                  value={salaryDraft.employee_id}
                >
                  <option value="">Chọn nhân viên</option>
                  {salaryConfigurations.map(
                    (item: EmployeeSalaryConfig): React.JSX.Element => (
                      <option key={item.employee_id} value={item.employee_id}>
                        {item.employee_name} · {roleLabel(item.role)}
                      </option>
                    ),
                  )}
                </select>
                {salaryQuery.hasNextPage ? (
                  <button
                    className="mt-2 text-xs font-semibold text-blue-700"
                    disabled={salaryQuery.isFetchingNextPage}
                    onClick={() => void salaryQuery.fetchNextPage()}
                    type="button"
                  >
                    {salaryQuery.isFetchingNextPage ? "Đang tải..." : "Tải thêm nhân viên"}
                  </button>
                ) : null}
              </label>
              <label className="text-sm font-semibold text-slate-700">
                ...
                <input
                  ...
                  required
                  value={salaryDraft.monthly_amount}
                />
              </label>
              <label className="text-sm font-semibold text-slate-700">
                Tiền tệ
                <input
                  className="mt-2 min-h-11 w-full rounded-xl border-slate-300 uppercase"
                  maxLength={3}
                  onChange={(event): void =>
                    setSalaryDraft((current): EmployeeSalaryRateCreateReq => ({
                      ...current,
              ...
              <label className="text-sm font-semibold text-slate-700">
                ...
                <input
                  ...
                  required
                  type="date"
                  value={salaryDraft.effective_from}
                />
              </label>
              <button
              ...
              {salaryMutation.error ? (
                <p className="text-sm font-semibold text-red-700 lg:col-span-5">
                  {friendlyApiError(
                    salaryMutation.error,
                    "Không thể lưu mức lương. Ngày hiệu lực có thể đã bị trùng.",
                  )}
                </p>
              ) : null}
            </form>
          ...
          <div className="panel overflow-hidden">
            <div className="border-b border-slate-200 px-5 py-4">
              <h2 className="font-black text-slate-950">
                Lương tháng tại chi nhánh đang chọn
              </h2>
              <p className="mt-1 text-sm text-slate-500">
                Mỗi thay đổi tạo một phiên bản mới; báo cáo tự phân bổ theo số
                ngày của từng tháng.
              </p>
            </div>
            {salaryQuery.isPending ? (
              ...
              <p className="m-5 text-sm text-red-700">
                {friendlyApiError(
                  salaryQuery.error,
                  ...
                  (item: EmployeeSalaryConfig): React.JSX.Element => (
                    ...
                    >
                      <div>
                        ...
                        <p className="mt-1 text-sm text-slate-500">
                          {item.employee_code} · {roleLabel(item.role)}
                        </p>
                      </div>
                        ...
                        {item.monthly_amount && item.currency ? (
                          <>
                            ...
                            <p className="mt-1 text-xs text-slate-500">
                              ...
                              Hiệu lực {item.effective_from}
                              {item.effective_to
                                ? ` đến ${item.effective_to}`
                                : " trở đi"}
                            </p>
                          </>
                        ) : (
                          <p className="font-semibold text-amber-700">
                            Chưa cấu hình lương tháng
                          </p>
                        )}
                      </div>
                    </article>
              ...
              {salaryQuery.hasNextPage ? (
                <div className="border-t border-slate-100 p-4 text-center">
                  <button
                    ...
                    type="button"
                  >
                    {salaryQuery.isFetchingNextPage ? "Đang tải..." : "Tải thêm cấu hình lương"}
                  </button>
                </div>
              ) : null}
            </div>
