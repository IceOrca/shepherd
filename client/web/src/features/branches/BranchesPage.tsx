          ...
          {editor ? (
            <form className="surface-card p-5 sm:p-6" onSubmit={submitUpdate}>
              ...
              <div className="mt-6 space-y-5">
                ...
                <label className="block"><span className="text-sm font-bold text-slate-800">Trạng thái</span><span className="mt-1 block text-xs leading-5 text-slate-500">Doanh nghiệp phải luôn giữ ít nhất một chi nhánh hoạt động; chi nhánh còn nghiệp vụ chưa hoàn tất cũng không thể vô hiệu hóa.</span><select className="mt-2 min-h-11 w-full rounded-xl border-slate-300" value={editor.status} onChange={(event): void => setEditor({ ...editor, status: event.target.value })}><option value="active">Hoạt động</option><option value="disabled">Vô hiệu hóa</option></select></label>
              </div>
