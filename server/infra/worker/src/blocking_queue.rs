    ...
    H: Fn(T, CancellationToken) + Send + Sync + 'static,
{
    ...
    loop {
        ...
        active_tasks.spawn_blocking(move || {
            let _permit = permit;
            let span = info_span!("blocking_queue_task", queue.name = %task_name);
            let _entered = span.enter();
            debug!(queue.name = %task_name, "Blocking queued task started");
            handler(task, cancellation);
            debug!(queue.name = %task_name, "Blocking queued task finished");
        });
    }
