...
use super::{KeycloakAuth, KeycloakPrincipal};
...
pub async fn require_authenticated(
    State(auth): State<Arc<KeycloakAuth>>,
    mut request: Request,
    next: Next,
) -> Result<Response, StatusCode> {
    let token =
        access_token(request.headers(), auth.config().accept_forwarded_access_token).ok_or(StatusCode::UNAUTHORIZED)?;
    let principal = auth.validate_access_token(token).await.map_err(|error| {
