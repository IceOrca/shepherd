...
fn write_payroll_summary_sheet(workbook: &mut Workbook, reports: &[PayrollReport]) -> Result<(), XlsxError> {
    ...
    for (offset, (currency, amount)) in totals.iter().enumerate() {
        ...
        write_money_scaled(worksheet, row, 4, amount.recorded_advance_deduction)?;
        write_money_scaled(worksheet, row, 5, amount.suggested_advance_deduction)?;
        write_money_scaled(worksheet, row, 6, amount.estimated_net_pay)?;
    }
    configure_money_columns(worksheet, 0, 6)?;
    if !totals.is_empty() {
        worksheet.autofilter(0, 0, checked_row(totals.len() - 1, 1)?, 6)?;
    }
    Ok(())
}
...
fn write_payroll_detail_sheet(workbook: &mut Workbook, reports: &[PayrollReport]) -> Result<(), XlsxError> {
    ...
    for report in reports {
        for line in &report.lines {
            ...
            for (column, value) in [
                ...
                (8, &line.gross_pay),
                (9, &line.recorded_advance_deduction),
                (10, &line.outstanding_advance_due),
                (11, &line.suggested_advance_deduction),
                (12, &line.estimated_net_pay),
            ] {
            ...
            if line.attendance_overlap_count > 0 {
                ...
                worksheet.write_number(row, 13, 0.0)?;
            }
            row = next_row(row)?;
        }
    }
    ...
    worksheet.set_column_width(13, 23.0)?;
    if row > 1 {
