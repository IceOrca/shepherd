...
#[derive(Debug, Deserialize, TS)]
pub struct FinancialReportExportRequest {
...
async fn export_report_xlsx(
    ...
    Json(payload): Json<FinancialReportExportRequest>,
    ...
    let tenant_name: String = context
        .db
        .run_with_tenant(user.tenant_id, async move |connection| {
...
fn validate_export_request(
    ...
    payload: &FinancialReportExportRequest,
    permission: &str,
) -> Result<(), StatusCode> {
