-- Make upgrades tolerant of duplicate reclose snapshots created by older
-- application code while preserving every append-only payment row.
DROP INDEX IF EXISTS hr_employee_profit_share_payments_month_recipient_uq;
...
AS $$
    WITH exact_month AS (
        ...
        JOIN LATERAL (
            SELECT DISTINCT ON (candidate.employee_id, candidate.currency)
                   candidate.*
            FROM hr_employee_profit_share_payments AS candidate
            WHERE candidate.tenant_id = target_tenant_id
              AND candidate.branch_id = target_branch_id
              AND candidate.payroll_period_start = exact_month.period_start
            ORDER BY candidate.employee_id, candidate.currency,
