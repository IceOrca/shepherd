...
CREATE TABLE hr_employee_compensations (
    ...
    CONSTRAINT hr_employee_compensations_employee_branch_tenant_fk
...
CREATE INDEX hr_employee_compensations_tenant_employee_dates_idx
    ON hr_employee_compensations (tenant_id, branch_id, employee_id, effective_from DESC, effective_to);

CREATE TABLE payroll_branch_rate_rules (
    id UUID PRIMARY KEY,
    tenant_id UUID NOT NULL REFERENCES tenants (id) ON DELETE RESTRICT,
    code TEXT NOT NULL,
    name TEXT NOT NULL,
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    created_by_account_id UUID NOT NULL,
    CONSTRAINT payroll_branch_rate_rules_tenant_id_id_uq UNIQUE (tenant_id, id),
    CONSTRAINT payroll_branch_rate_rules_branch_tenant_fk
        FOREIGN KEY (tenant_id, branch_id)
        REFERENCES branches (tenant_id, id)
        ON DELETE RESTRICT,
    CONSTRAINT payroll_branch_rate_rules_employee_tenant_fk
        FOREIGN KEY (tenant_id, employee_id)
        REFERENCES hr_employees (tenant_id, id)
        ON DELETE RESTRICT,
    CONSTRAINT payroll_branch_rate_rules_created_by_tenant_fk
        FOREIGN KEY (tenant_id, created_by_account_id)
        REFERENCES accounts (tenant_id, id)
        ON DELETE RESTRICT,
    CONSTRAINT payroll_branch_rate_rules_code_valid CHECK (
        code = lower(btrim(code))
        AND char_length(code) BETWEEN 2 AND 63
        AND code ~ '^[a-z0-9]([a-z0-9_-]*[a-z0-9])?$'
    ),
    CONSTRAINT payroll_branch_rate_rules_name_valid CHECK (
        name = btrim(name) AND char_length(name) BETWEEN 1 AND 200
    ),
    CONSTRAINT payroll_branch_rate_rules_values_valid CHECK (
        base_multiplier >= 1 AND hourly_adjustment >= 0
    ),
    CONSTRAINT payroll_branch_rate_rules_dates_valid CHECK (
        effective_to IS NULL OR effective_to >= effective_from
    ),
    UNIQUE (tenant_id, code, effective_from)
);

CREATE INDEX payroll_branch_rate_rules_lookup_idx
    ON payroll_branch_rate_rules (
        tenant_id, branch_id, employee_id, effective_from DESC, effective_to, priority DESC
    )
    WHERE is_active;
...
CREATE TABLE payroll_time_band_rules (
    ...
    CONSTRAINT payroll_time_band_rules_branch_tenant_fk
        FOREIGN KEY (tenant_id, branch_id)
        REFERENCES branches (tenant_id, id)
        ON DELETE RESTRICT,
...
CREATE INDEX payroll_time_band_rules_lookup_idx
    ON payroll_time_band_rules (tenant_id, branch_id, effective_from DESC, effective_to, priority DESC)
    WHERE is_active;
...
CREATE TABLE payroll_overtime_rules (
    ...
    CONSTRAINT payroll_overtime_rules_branch_tenant_fk
        FOREIGN KEY (tenant_id, branch_id)
        REFERENCES branches (tenant_id, id)
        ON DELETE RESTRICT,
...
CREATE INDEX payroll_overtime_rules_lookup_idx
    ON payroll_overtime_rules (tenant_id, branch_id, threshold_minutes, effective_from DESC, effective_to)
    WHERE is_active;
...
CREATE TABLE payroll_runs (
    ...
    CONSTRAINT payroll_runs_branch_tenant_fk
        FOREIGN KEY (tenant_id, branch_id)
        REFERENCES branches (tenant_id, id)
        ON DELETE RESTRICT,
    ...
    UNIQUE (tenant_id, branch_id, period_start, period_end, currency)
);

CREATE INDEX payroll_runs_tenant_period_idx
    ON payroll_runs (tenant_id, branch_id, period_start DESC, period_end DESC);
...
CREATE TABLE payroll_run_lines (
    ...
    CONSTRAINT payroll_run_lines_attendance_tenant_fk
        ...
        ON DELETE RESTRICT,
    CONSTRAINT payroll_run_lines_branch_tenant_fk
        FOREIGN KEY (tenant_id, branch_id)
        REFERENCES branches (tenant_id, id)
        ON DELETE RESTRICT,
...
CREATE TABLE payroll_employee_results (
    ...
    branch_amount NUMERIC(19, 4) NOT NULL,
    ...
    CONSTRAINT payroll_employee_results_branch_tenant_fk
        FOREIGN KEY (tenant_id, branch_id)
        REFERENCES branches (tenant_id, id)
        ON DELETE RESTRICT,
    ...
    CONSTRAINT payroll_employee_results_values_valid CHECK (
        ...
        AND gross_amount = base_amount + branch_amount + time_amount + overtime_amount
    ),
    CONSTRAINT payroll_employee_results_currency_valid CHECK (
        currency = upper(currency) AND currency ~ '^[A-Z]{3}$'
    ),
    UNIQUE (tenant_id, branch_id, payroll_run_id, employee_id)
);
...
ALTER TABLE payroll_branch_rate_rules ENABLE ROW LEVEL SECURITY;
ALTER TABLE payroll_branch_rate_rules FORCE ROW LEVEL SECURITY;
CREATE POLICY payroll_branch_rate_rules_tenant_isolation ON payroll_branch_rate_rules
    USING (tenant_id = shepherd_current_tenant_id() AND shepherd_branch_visible(branch_id))
    WITH CHECK (tenant_id = shepherd_current_tenant_id() AND shepherd_branch_visible(branch_id));
...
ALTER TABLE hr_employee_compensations ALTER COLUMN branch_id SET DEFAULT shepherd_current_branch_id();
ALTER TABLE payroll_branch_rate_rules ALTER COLUMN branch_id SET DEFAULT shepherd_current_branch_id();
ALTER TABLE payroll_time_band_rules ALTER COLUMN branch_id SET DEFAULT shepherd_current_branch_id();
ALTER TABLE payroll_overtime_rules ALTER COLUMN branch_id SET DEFAULT shepherd_current_branch_id();
ALTER TABLE payroll_runs ALTER COLUMN branch_id SET DEFAULT shepherd_current_branch_id();
ALTER TABLE payroll_run_lines ALTER COLUMN branch_id SET DEFAULT shepherd_current_branch_id();
ALTER TABLE payroll_employee_results ALTER COLUMN branch_id SET DEFAULT shepherd_current_branch_id();
...
WHERE role.code IN ('tenant_owner', 'executive_manager', 'branch_manager')
