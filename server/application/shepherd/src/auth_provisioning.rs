...
use infra_auth::ext_service::auth_admin::{
    AuthAccountAccessContext, AuthProvisioner, AcctProvisionContext, AcctProvisionErr,
};
...
#[derive(Debug)]
pub struct AppAuthProvisioner;

#[async_trait]
impl AuthProvisioner for AppAuthProvisioner {
    async fn provision(
        &self,
...
fn employee_branch(
    ...
    let branch_id: Uuid = current_branch_id
        ...
        .ok_or_else(|| {
            ...
            AcctProvisionErr::new("employee_branch_assignment_missing")
        })?;
    Ok(branch_id)
}
