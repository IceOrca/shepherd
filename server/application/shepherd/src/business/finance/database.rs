...
impl FinanceRepo for FinanceDb {
    ...
    async fn list_expenses(
        ...
        let sql_query: String = format!(
            "{EXPENSE_SELECT} WHERE claim.tenant_id = $1 AND ($2 OR claim.submitted_by_account_id = $3 OR payer.account_id = $3) AND ($4::TEXT IS NULL OR claim.status = $4) AND ($5::TEXT IS NULL OR lower(claim.description) LIKE '%' || $5 || '%' OR lower(category.display_name) LIKE '%' || $5 || '%' OR lower(COALESCE(payer.display_name, '')) LIKE '%' || $5 || '%' OR lower(submitter.username) LIKE '%' || $5 || '%') AND ($6::DATE IS NULL OR (claim.paid_on, claim.created_at, claim.id) < ($6, $7::TIMESTAMPTZ, $8::UUID)) {EXPENSE_GROUP_ORDER} LIMIT $9"
        );
