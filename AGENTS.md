...
Expense/advance creation and correction replays must check current subject ownership and effective branch/self read permissions, including account allow/deny overrides, before returning the current projection. A historical idempotency key is not read authority after reassignment. Share this rule across both finance subjects; a correction key bound to another subject remains a conflict.
