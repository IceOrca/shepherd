...
- Every branch-owned record is additionally protected by the validated active branch context. Browser requests send `X-Shepherd-Branch-Id`; middleware accepts it only when PostgreSQL-authoritative account access contains that branch, and SQL transactions set `app.branch_id` for RLS.
...
- `account_branch_assignments`: database-authoritative account access to branches; tenant owners are tenant-wide, executive managers may have many branches, and branch manager/supervisor/staff accounts have exactly one.
...
- `tenant_owner` is tenant-wide and has no `account_branch_assignments` rows. `executive_manager` has one or more assigned branches. Each `branch_manager`, `supervisor`, and `staff` account has exactly one branch. Cardinality comes from `auth_role_branch_assignment_rules` and the database guard, not a closed reusable Rust role enum.
...
- `AuthenticatedUser` remains the request identity boundary and includes tenant/account IDs, username, optional application-owned email, primary role, roles, permissions, authorized branch IDs, and the validated active branch ID.
- Keep only `tid` in the GoTrue JWT. Never put role or branch authority in JWT claims. Branch access comes from PostgreSQL and the bounded authenticated-user cache; the browser's `X-Shepherd-Branch-Id` is only a requested active context and must be rejected when it is absent from the resolved account's authorized branch IDs.
...
- **Auth administrator**: pre-provision and disable external Auth users while maintaining Shepherd account and branch mappings.
...
The frontend persists one active branch per tenant, sends it on application API calls, and displays a selector for multi-branch users. Switching branches must update the API client immediately and invalidate TanStack Query data so records from the previous branch are not reused. A single-branch account sees its branch without a switcher. Frontend selection is usability state only; middleware authorization and PostgreSQL RLS remain authoritative.
