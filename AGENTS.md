...
- A requested salary advance has no approval or disbursement actor yet. Finance SQL projections must explicitly preserve nullable usernames from those left joins so create and list responses return `null` until the corresponding lifecycle transition instead of failing response decoding.
- Expense and salary-advance access is permission-driven and branch-bounded. An employee-paid expense belongs to its payer; a company-funded expense belongs to its submitting account; a salary advance belongs to its employee regardless of which authorized account created it. `*.self.read` returns only that subject's records. `business.expenses.read` and `hr.salary_advances.read` expose read-only records inside the validated active branch; they never imply create or correction authority. `business.expenses.self.correct` and `hr.salary_advances.self.correct` permit only the subject to correct its still-unconfirmed record. `business.expenses.manage` and `hr.salary_advances.manage` separately permit creating or correcting another employee's unconfirmed record. The server revalidates both the existing and proposed employee subject inside the locked correction transaction, and the database correction trigger independently applies effective role grants plus account allow/deny overrides. Built-in branch/executive managers receive managed-branch read plus self-service defaults, not manage, approve, settle, disburse, recover, or terminal-correction defaults. The tenant owner receives the manage and decision defaults and may operate across branches only through an explicit validated branch context. These are configurable grants, never role-name checks in application code.
- Reimbursements and salary-advance recoveries are immutable settlement facts. Manual expense reimbursement (**Hoàn trả**) and manual advance repayment (**Thu hồi**) remain independent actions. Their server-owned cash timestamp is converted through the fixed branch time zone, and insertion is rejected when that local cash month is closed; manual cash settlement must never change an already locked operating report. Period-close settlements use the separate internal sources `payroll_settlement` and `payroll_deduction`, snapshot both the exact `payroll_inclusion_on` and branch-local `payroll_period_start`, and reference the close event that created them. The exact date keeps arbitrary partial-month report intervals stable before and after close. A source correction may not change payer, employee, currency, or settled monetary identity after settlement. A settlement mistake must be represented by a separately authorized compensating settlement entry; never update or delete the original cash movement.
...
- Monthly salary inserts and effective-date changes must reject every affected closed month, including current/future closed months. Employee hire/termination-date edits check the symmetric difference of old/new employment coverage intersected with configured salary coverage. Use PostgreSQL date multiranges and the latest period decision, not an unbounded generated calendar or any historical close event. Unaffected days stay editable; an explicit recorded reopen permits correction.
- Branch time zones are fixed after the first expense, advance, monthly salary, reconciled result, settlement, or financial-period decision. Keep branch name/status edits and time-zone no-ops available. The database guard checks the exact target branch even when tenant-wide branch administration has a different active branch; any internal branch-context expansion must retain tenant RLS and restore the previous context.
- Financially guarded default-workflow mutations lock the active branch before employee, report, assignment, or finance-projection rows, consistently with financial close. Reuse the application-owned business database lock helper; never acquire shared branch locks and later upgrade them in concurrent urgent starts. Ordinary reads do not take this mutation barrier.
...
Expense correction is `POST /api/business/finance/expenses/{expense_id}/correct`; salary-advance correction is `POST /api/business/finance/salary-advances/{advance_id}/correct`. Each request requires the latest `expected_revision_id`, a normalized correction reason, and an `Idempotency-Key`. A stale revision returns conflict. The subject may repair its own unconfirmed record only with the corresponding `*.self.correct` permission; managing another employee's unconfirmed record requires the separate `*.manage` permission. Changing an approved/disbursed/recovered financial record requires the dedicated `*.correct` permission. The server locks the projection, revalidates authorization, the proposed subject, and settlement constraints, sets transaction-local revision metadata, updates the projection, and lets the database trigger append the new full snapshot in the same transaction. History endpoints apply the same subject/managed-branch read scope to retained snapshots.

Expense/advance creation and correction replays must check current subject ownership and effective branch/self read permissions, including account allow/deny overrides, before returning the current projection. Creation replay lookup occurs under the branch lock before a new insert is attempted, so an accepted request remains replayable after its financial month closes. Compare a creation retry with revision 1's complete original payload and reject key reuse for different input; a later correction to the current projection does not change that replay identity. A historical idempotency key is not read authority after reassignment. Share this rule across both finance subjects; a correction key bound to another subject remains a conflict.

The browser retains one idempotency key for an unchanged finance mutation until success or explicit dialog dismissal. A lost response followed by an unchanged retry must reuse that key; editing the payload starts a new attempt. Financial-period decisions compare the replayed month, target status, expected revision, and reason. Monthly-salary creation compares employee, exact numeric amount, currency, and effective date. Reusing either key for a different command conflicts. An employee's unconfirmed expense/advance correction authority follows the current and proposed business subject, even when an authorized manager or owner originally submitted the record; original submitter/requester identity is audit provenance, not self-service ownership.

Payroll employee metadata must produce one row per employee identity before joining currency totals. Prefer the current branch-visible HR record; use a distinct saved profit-share identity only when that employee is absent from the current branch projection. Mutable name/code/role differences between current HR data and closed snapshots must never duplicate an employee or multiply money. Preserve the saved snapshot unchanged.

Financial periods are branch-local calendar months and are also the current payroll-lock boundary. `business_financial_period_events` is an append-only open/closed decision stream; absence of an event means open. Closing or reopening a month requires `finance.periods.manage`, a reason, `expected_revision_number`, and an idempotency key. The repository locks the authoritative branch row before allocating the next revision so concurrent decisions cannot fork. Closing first rejects approved staffing/HR-attendance overlaps, then atomically appends the close event, reimburses every remaining approved employee-paid expense whose `payroll_inclusion_on` is in the month, and recovers every remaining disbursed salary advance whose `payroll_inclusion_on` is in the month. The linked immutable settlements transition advances to `recovered`; a fully reimbursed expense is derived from zero outstanding balance. Reopening appends a new event and never reverses or deletes settlements from an earlier close; a later close settles only new eligible remaining balances. Expense and advance correction checks both the old and proposed `paid_on` and `payroll_inclusion_on` months in PostgreSQL and rejects a change when any affected month is closed. A completed urgent-work session checks every customer-local date covered by its interval against the latest period decisions; open endpoint months do not permit it to cross a closed intermediate month.
...
- Production Caddy runs in Compose, serves the UI and `/api/*` on `SHEPHERD_WEB_ORIGIN_PROD`, and serves Auth on a separate `https://${AUTH_DNS_NAME_PROD}` site. Its immutable image contains the Vite build and production Caddyfile. It strips `/auth/v1` only on the private Docker-network hop. Never publish port 9999, PostgreSQL, Redis, or the Shepherd server directly.
...
- Production publishes only the Compose Caddy container on TCP 80/443 and UDP 443. Caddy uses wildcard listeners, persistent `caddy_data` and `caddy_config` volumes, and private service-name upstreams. `PUBLIC_VPS_IPV4_PROD` is for external DNS validation and must not become a Caddy `bind` address.
- Production Caddy pins Let's Encrypt's production ACME directory and requires a monitored `ACME_EMAIL_PROD`. Preserve `caddy_data` across releases so Caddy can renew certificates automatically; never add Certbot, host certificate copies, or a competing renewal cron job.
- Production application images are built only through the PC/CI `compose.build.yaml` overlay, with SBOM and maximum provenance attestations, then pushed under non-`latest` release tags. The normal `compose.yaml + compose.prod.yaml` VPS model must contain no build contexts and starts with `--pull always --no-build`. Private Docker Hub access uses a read-only VPS token; build overlays and registry write tokens never belong on the VPS.
...
- Declare static endpoint permission requirements on each Axum method route with the shared `PermissionRouteExt::{require_one, require_any, require_all}` middleware API. Do not repeat equivalent permission helpers in feature handlers. Keep record ownership, active-branch scope, lifecycle/revision rules, payload-dependent permissions, and PostgreSQL-authoritative mutation rechecks in core/database code or the handler that supplies those domain inputs; route middleware is only the coarse cached request gate.
...
1. Copy `deploy/secrets_example/example.env` to the operator-owned environment file. It contains only non-secret Compose settings. Copy every `*.prod.example` secret to the exact filename under the absolute `SVR_SECRETS_DIR`, replace every placeholder, and restrict each secret to mode `0600` or `0400`. Database passwords must use only URL-safe ASCII characters because the minimal entrypoints assemble PostgreSQL URLs without exposing the passwords through Compose.
2. Create the public web and Auth DNS records and wait until both resolve to the declared VPS address.
3. Run `scripts/check-production-config.sh /etc/shepherd/shepherd.env`. It rejects documentation placeholders, missing or over-permissive secret files, Auth secrets accidentally placed in the Compose environment, an inconsistent Auth URL chain, an invalid merged Compose model, and an invalid production Caddyfile without printing secret values.
4. On a trusted PC/CI machine, build and push the runtime and Caddy/web images with `compose.build.yaml` and the default empty `SHEPHERD_CARGO_FEATURES`. The edge image embeds the Vite artifact and production Caddyfile; production never runs Vite's preview server or copies frontend assets onto the VPS host.
5. On the VPS, use only `compose.yaml + compose.prod.yaml`: pull images and start with `--pull always --no-build`. PostgreSQL health gates the idempotent `postgres-bootstrap` role/schema job, which gates the embedded `database-migrate` job; only a successful migration then permits GoTrue and Shepherd to start. Compose Caddy reaches GoTrue and Shepherd through private service-name upstreams. Only Caddy publishes host ports; PostgreSQL, Redis, GoTrue, and Shepherd have no host ports.
6. Initialize the platform administrator once with the profile-gated production `system-admin-init` binary, then create the first tenant/owner through `/system-admin` or use the protected tenant-bootstrap CLI. Neither tool inherits a development image or credential file in the production merge.
7. Run `scripts/check-production-auth-edge.sh` after Compose Caddy obtains its public certificates to verify DNS, public TLS, `disable_signup=true`, the GoTrue settings endpoint, and browser CORS preflight.
8. Verify password login, application-account mapping through `/api/me`, logout, refresh, and each enabled social-provider callback. Existing sessions from a previous issuer are not expected to survive.
...
Keep images minimal. The server uses Rust Trixie: do not add `build-essential`, `libpq-dev`, or `postgresql-client`; access and migrations use SQLx, not Diesel. Add OS packages only for demonstrated needs. Run manual `psql` only in `postgres-db` (PostgreSQL Alpine), never the server image.
