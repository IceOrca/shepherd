...
- `server/crates/foundation/`: reusable backend capabilities. `kernel` contains neutral primitives; `infra/postgres` and `infra/redis` are thin adapters; `auth` is an independent vertical feature; and `host` owns shared Axum policies and composition types.
- `server/crates/applications/hrm/`: HRM domain, persistence adapters, and Axum routes.
- `server/runtime/`: composition root and the `shepherd-server`/`shepherd-openapi` binaries.
...
Run backend commands through the already-running development containers:
...
Run frontend commands through the client container:
...
Rust uses edition 2024 with `unsafe_code = "forbid"`. Keep reusable capabilities in `foundation`, HRM business code in `applications/hrm`, and process assembly in `runtime`. Use `snake_case` for Rust modules/functions and `PascalCase` for types. Avoid `unwrap()` and direct indexing; return or map errors explicitly.
...
`foundation-auth` may depend on `foundation-kernel`, `foundation-postgres`, and `foundation-redis`, but never on `foundation-host`. `foundation-host` includes auth in its default Cargo features and may re-export it. `HostContext` contains shared adapters and services; applications return `AppRoutes` groups instead of putting application services into the foundation. Keep logging, audit, rate limiting, trusted-IP extraction, and HTTP policy in `host`; keep debugging primitives in `kernel`.

## Backend Auth Architecture SDD

### Purpose

The backend auth design is a hybrid model:
...
No frontend test runner is configured yet. For UI changes, at minimum run `npm run lint` and `npm run build`.
