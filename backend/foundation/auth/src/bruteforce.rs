...
use foundation_kernel::request::OriginatorIp;
...
impl BruteForceGuard {
    pub fn new_arc(redis: Arc<RedisAdapter>) -> Arc<Self> {
        ...
        let backend: String = std::env::var("BRUTE_FORCE_BACKEND").unwrap_or_else(|_| "dashmap".to_string());
        let ops: DynBruteForceOps = if backend == "redis" {
    ...
    impl BruteForceTracker {
        pub(super) fn new_arc(config: BruteForceConfig, redis_pool: Arc<RedisAdapter>) -> Arc<Self> {
            let key_prefix: String =
                std::env::var("BRUTE_FORCE_REDIS_PREFIX").unwrap_or_else(|_| "foundation:bruteforce:".to_string());
            log_info!("BruteForce Redis backend initialized with key_prefix={}", key_prefix);
            Arc::new(Self {
                redis_pool,
                config,
                key_prefix,
