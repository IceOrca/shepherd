...
impl AuthSessionHandle {
    pub fn new_arc(redis: Arc<RedisAdapter>) -> Arc<Self> {
        ...
        let key_prefix: String =
            std::env::var("AUTH_SESSION_REDIS_PREFIX").unwrap_or_else(|_| "foundation:host:auth:".to_string());
    ...
    pub async fn create_session(
        ...
        log_notice!(
            "AuthSession created: backend=redis tenant_id={} account_id={} sid={} jti={} ttl={}s max={}",
            tenant_id,
            account_id,
            sid,
            jti,
    ...
    pub async fn rotate_session(
        ...
        if status == "not_found" {
            log_notice!(
                "AuthSession refresh rejected: backend=redis sid={} reason=not_found",
                sid
            );
            return Err(AuthSessionError::RefreshNotFound);
        }
        if status == "expired" {
            log_notice!("AuthSession refresh rejected: backend=redis sid={} reason=expired", sid);
            return Err(AuthSessionError::SessionExpired(revoked_access_token));
        }
        if status == "idle_timeout" {
            log_notice!(
                "AuthSession refresh rejected: backend=redis sid={} reason=idle_timeout",
                sid
            );
            return Err(AuthSessionError::SessionExpired(revoked_access_token));
        }
        if status == "mismatch" {
            log_warn!(
                "AuthSession refresh token mismatch; session revoked: backend=redis sid={} revoked_access={:?}",
                sid,
                revoked_access_token
            );
        ...
        log_notice!(
            "AuthSession refresh rotated: backend=redis tenant_id={} account_id={} sid={} new_access_jti={}",
            entry.tenant_id,
            entry.account_id,
            entry.sid,
            jti
    ...
    pub async fn validate_session(
        ...
        if status == "expired" || status == "idle_timeout" {
            log_notice!(
                "AuthSession refresh rejected: backend=redis sid={} reason={}",
                sid,
                status
            );
            return Err(AuthSessionError::SessionExpired(revoked_access_token));
    ...
    pub async fn revoke_session(
        ...
        match result {
            Ok(result) => {
                ...
                log_notice!(
                    "AuthSession revoked refresh session: backend=redis tenant_id={} account_id={} sid={} revoked_access={:?}",
                    tenant_id,
                    account_id,
                    sid,
                    access_token
            ...
            Err(err) => {
                log_error!(
                    "Failed to revoke Redis auth refresh session: backend=redis tenant_id={} account_id={} sid={} error={}",
                    tenant_id,
                    account_id,
                    sid,
                    err
    ...
    pub async fn revoke_all_sessions(
        ...
        match result {
            Ok(result) => {
                ...
                log_notice!(
                    "AuthSession revoke-all complete: backend=redis tenant_id={} account_id={} access_token_count={}",
                    tenant_id,
                    account_id,
                    access_tokens.len()
                );
            ...
            Err(err) => {
                log_error!(
                    "Failed to revoke all Redis auth refresh sessions: backend=redis tenant_id={} account_id={} error={}",
                    tenant_id,
                    account_id,
                    err
                );
    ...
    async fn connection(&self) -> Result<MultiplexedConnection, AuthSessionError> {
        self.redis_pool.connection().await.map_err(|err: redis::RedisError| {
            log_error!("Failed to connect to Redis for auth session backend: {}", err);
            AuthSessionError::BackendUnavailable
        })
    }
